...
>(({ className, ...props }, ref) => (
	<ToastPrimitives.Viewport
		ref={ref}
		className={cn(
			"fixed top-10%  left-1/2 transform -translate-x-1/2 z-[100] flex w-[320px] flex-col-reverse p-4 sm:top-10% sm:left-1/2 sm:transform sm:-translate-x-1/2 sm:flex-col",
			className
		)}
		{...props}
	/>
...
>(({ className, variant, ...props }, ref) => (
	<ToastPrimitives.Close
		ref={ref}
		className={cn(
			"cursor-pointer absolute right-4 top-4 rounded-md text-foreground/50 transition-opacity hover:text-foreground focus:opacity-100 focus:outline-none group-hover:opacity-100",
			className
		)}
		toast-close=''
		{...props}
